...
#include <sensor_ros_plugins/DVLROSPlugin.hh>
...
void DVLROSPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  for (int i = 0; i < 4; i++)
    this->dvlBeamMsgs.push_back(sensor_ros_plugins_msgs::DVLBeam());
  ...
  this->rosSensorOutputPub =
    this->rosNode->advertise<sensor_ros_plugins_msgs::DVL>(
