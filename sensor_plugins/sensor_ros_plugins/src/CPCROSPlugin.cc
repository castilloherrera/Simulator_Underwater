...
#include <sensor_ros_plugins/CPCROSPlugin.hh>
...
void CPCROSPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  {
    if (salinityUnit.compare(sensor_ros_plugins_msgs::Salinity::PPT) == 0)
      this->waterSalinityValue = 35.0;
    else if (salinityUnit.compare(sensor_ros_plugins_msgs::Salinity::PPM) == 0)
  ...
  this->rosSensorOutputPub = this->rosNode->advertise<
    sensor_ros_plugins_msgs::ChemicalParticleConcentration>(
      this->sensorOutputTopic, 1);

  this->salinityPub = this->rosNode->advertise<
    sensor_ros_plugins_msgs::Salinity>(salinityTopic, 1);
