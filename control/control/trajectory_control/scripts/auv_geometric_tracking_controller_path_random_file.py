...
from control_interfaces import DPControllerPathRandomFile
import tf2_ros
from tf.transformations import quaternion_matrix


class AUVGeometricTrackingControllerPathRandomFile:
    def __init__(self):
        self.namespace = rospy.get_namespace().replace('/', '')
        rospy.loginfo('Initialize control for vehicle <%s>' % self.namespace)

        self.local_planner = DPControllerPathRandomFile(full_dof=True, thrusters_only=False,
...
if __name__ == '__main__':
    ...
    rospy.init_node('auv_geometric_tracking_controller_path_random_File')

    try:
        node = AUVGeometricTrackingControllerPathRandomFile()
        rospy.spin()
