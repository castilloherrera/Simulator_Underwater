...
from control_interfaces import DPControllerPathNPolygon
import tf2_ros
from tf.transformations import quaternion_matrix


class AUVGeometricTrackingControllerNPolygon:
    def __init__(self):
        self.namespace = rospy.get_namespace().replace('/', '')
        rospy.loginfo('Initialize control for vehicle <%s>' % self.namespace)

        self.local_planner = DPControllerPathNPolygon(full_dof=True, thrusters_only=False,
...
if __name__ == '__main__':
    ...
    rospy.init_node('auv_geometric_tracking_controller_NPolygon')

    try:
        node = AUVGeometricTrackingControllerNPolygon()
        rospy.spin()
