...
class DPControllerPathNPolygon(object):
    """
    ...
    def __init__(self, full_dof=False, stamped_pose_only=False, thrusters_only=True):
        ...
        self._idle_polygon_center = None
        self._idle_z = None

        self._logger.info('Max. forward speed [m/s]=%.2f' % self._max_forward_speed)

        self._idle_radius = rospy.get_param('~idle_radius', 30.0)
        ...
        self._services['start_n_polygon_trajectory'] = rospy.Service(
            'start_n_polygon_trajectory', InitNPolygonTrajectory,
            self.start_n_polygon)
    ...
    def start_n_polygon(self, request):
        if request.max_forward_speed <= 0 or request.n <= 0 or request.radius <= 0 or \
           request.n_points <= 0:
            self._logger.error('Invalid parameters to generate a regular n polygon trajectory')
            return InitNPolygonTrajectoryResponse(False)
        t = rospy.Time(request.start_time.data.secs, request.start_time.data.nsecs)
        if t.to_sec() < rospy.get_time() and not request.start_now:
            self._logger.error('The trajectory starts in the past, correct the starting time!')
            return InitNPolygonTrajectoryResponse(False)
        try:
            wp_set = waypoints.WaypointSet(
                inertial_frame_id=self.inertial_frame_id)
            success = wp_set.generate_regular_n_polygon(n=request.n,
            ...
            if not success:
                self._logger.error('Error generating regular n polygon trajectory from waypoint set')
                return InitNpolygonTrajectoryResponse(False)
            wp_set = self._apply_workspace_constraints(wp_set)
            if wp_set.is_empty:
            ...
            self._logger.info('============================')
            self._logger.info('REGULAR N POLYGON TRAJECTORY GENERATED FROM WAYPOINT INTERPOLATION')
            self._logger.info('============================')
            self._logger.info('Sides  = %.2f' % request.n)
            ...
            self._logger.info('============================')
            self._lock.release()
            return InitNPolygonTrajectoryResponse(True)
        except Exception, e:
            self._logger.error('Error while setting regular n polygon trajectory, msg=' + str(e))
            self.set_station_keeping(True)
            self.set_automatic_mode(False)
    ...
    def get_idle_n_polygon_path(self, n_points, n=10, radius=30):
        pose = deepcopy(self._vehicle_pose)
        if self._idle_polygon_center is None:
            frame = np.array([
