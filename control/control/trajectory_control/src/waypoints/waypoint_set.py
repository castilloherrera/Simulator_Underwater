...
from control_msgs.msg import WaypointSet as WaypointSetMessage
...
class WaypointSet(object):
    ...
    def generate_rectangle(self, height, width, origin, num_points, max_forward_speed,
                        theta_offset=0.0, heading_offset=0.0, append=False):
        if height <= 0:
            print 'Invalid heigth, value=', heigth
            return False

        if width <= 0:
            print 'Invalid width, value=', width
            return False
